...
// Physical: computed from spectrum, but RGB pipeline introduces inaccuracies.
// Artistic: adjusted from physical to better match ground truth.

//const vec3 SUN_IRRADIANCE = (vec3(225, 210, 205) / vec3(255.0)) * 1360.0; // Artistic
const vec3 SUN_IRRADIANCE = vec3(210, 190, 185); // Artistic
//const vec3 SUN_IRRADIANCE = vec3(2.1493e2, 1.9076e2, 1.841e2); // Physical

const vec3 RAYLEIGH_SCATTERING_BASE = mix(vec3(46.0, 95.0, 233.0), vec3(46.0, 89.0, 207.0), 0.7) / 255.0 * 0.03624; // Artistic
//const vec3 RAYLEIGH_SCATTERING_BASE = vec3(6.6049e-3, 1.2345e-2, 2.9413e-2); // Physical
